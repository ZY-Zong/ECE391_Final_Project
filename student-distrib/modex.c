 ...
 *   INPUTS: status_msg - a pointer to the string needed for subroutine draw_status_bar(char* status_msg)
 *   OUTPUTS: none
 *   RETURN VALUE: none
 *   SIDE EFFECTS: copies from the build buffer to video memory;
 *                 shifts the VGA display source to point to the new image
 *                 change the video memory for the status bar
 */
void
show_screen (char* status_msg)
{
...
/* draw_status_bar
 * This function calls subroutine text_to_image and splits graph value into planes.
 * Then push the plane values onto the video memory
 * Input: status_msg - pointer to the "sorted" string, ready to print
 * Output: None.
...
draw_vert_line (int x)
{
    ...
    // Adjust the x to the logical column value
    x += show_x;
    ...
    // Calculate the plane offset of the whole plane (since pixels on a verticle line share the same plane value)
    p_off = 3 - (x & 3);

    //Copy the image data into appropriate plane in build buffer
